...
### 7.8. HTML-контактлист: относительные пути не работают
`make_contact_sheet.py` v1.0.0 использовал относительные пути от корня репо,
но HTML лежит в `sessions/<story>/` — браузер не находит картинки на уровень выше.
**Решение (v1.0.1):** `file://` абсолютные URL через `Path.resolve().as_uri()`.
Не возвращаться на относительные пути.
